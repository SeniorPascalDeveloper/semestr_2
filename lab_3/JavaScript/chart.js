function createArrGraph(data, key) {
    ...
    for (let entry of groupObj) {
        ...
        const values = entry[1].map(d => d["Высота"]);
...
function drawBars(svg, data, scaleX, scaleY, attr, mode, color, offset) {
    ...
    svg.selectAll("." + mode)
        ...
        .enter()
        .append("rect")
        .attr("x", d => scaleX(d.labelX) + (offset === -1 ? 2 : band / 2))
        .attr("y", d => scaleY(d[mode]))
        .attr("width", band / 2 - 2)
        .attr("height", d =>
            attr.height - 2 * attr.marginY - scaleY(d[mode])
        )
        .attr("transform",
            `translate(${attr.marginX},${attr.marginY})`)
        .style("fill", color);
...
function drawDots(svg, data, scaleX, scaleY, attr, mode, color, offset) {
    ...
    svg.selectAll("." + mode)
        ...
        .attr("r", 5)
        .attr("cx", d =>
            scaleX(d.labelX) + band / 2 + offset * 8
        )
